...
me.hookall = function()
	...
	while true do
		...
		hasonupdate = frame.GetScript and frame:GetScript("OnUpdate")
		hasonevent = frame.GetScript and frame:GetScript("OnEvent")
		...
		allframes[frame].name = frame.GetName and frame:GetName() or ""
		...
		for key, value in allframes do
			...
			if value.numchildren == 0 then
				...
				if value.onupdate == nil and value.onevent == nil then
					...
					if not (key.GetName) or (key:GetName() == nil) then
						-- remove name from parent's list
						local parent = key.GetParent and key:GetParent() or false
						parentvalue = parent and allframes[key:GetParent()]
